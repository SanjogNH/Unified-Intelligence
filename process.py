...
def safe_gross_clicks(row):
    """
    Validate gross_clicks against ad_clicks.

    Returns the gross_clicks value when it is internally consistent
    (>= ad_clicks, since organic_clicks = gross - ad must be >= 0).
    Returns None when the field is missing OR when the source data
    is clearly malformed (gross < ad, or gross == 0 while ad > 0).

    None propagates through slim_sku → JSON drops the key →
    dashboard renders "—" instead of misleading zero values.
    """
    g = row.get("Gross Clicks")
    a = row.get("Ad Clicks")
    if g is None:
        return None
    try:
        ...
        return None
    # Both zero -> legitimately zero (truly inactive listing)
    if gf == 0 and af == 0:
        return 0.0
    # gross < ad is a data-integrity failure (organic can't be negative)
    if gf < af:
        return None
    return gf
...
def run(sales_path: str, city_path: str = None, ads_path: str = None,
    ...
    l3m_idx = {(r["Platform"], r["SKU"]): r for _, r in df_l3m.iterrows()}
    ...
    for _, s in df_sales.iterrows():
        ...
        row = {
            ...
            "gross_clicks":   safe_gross_clicks(ads_mtd),
            ...
            "lfm_gross_sales":     safe(ads_lfm.get("Gross Sales")),
            "lfm_gross_clicks":    safe_gross_clicks(ads_lfm),
            "lfm_ad_spend":        safe(ads_lfm.get("Ad Spend")),
            "lfm_ad_impressions":  safe(ads_lfm.get("Ad Impressions")),
            "lfm_ad_clicks":       safe(ads_lfm.get("Ad Clicks")),
            "lfm_ad_units":        safe(ads_lfm.get("Ad Units")),
            "lfm_ad_sales":        safe(ads_lfm.get("Ad Sales")),
            "l3m_gross_units":     safe(ads_l3m.get("Gross Units")),
            "l3m_gross_sales":     safe(ads_l3m.get("Gross Sales")),
            "l3m_gross_clicks":    safe_gross_clicks(ads_l3m),
            "l3m_ad_spend":        safe(ads_l3m.get("Ad Spend")),
            "l3m_ad_impressions":  safe(ads_l3m.get("Ad Impressions")),
            "l3m_ad_clicks":       safe(ads_l3m.get("Ad Clicks")),
            "l3m_ad_units":        safe(ads_l3m.get("Ad Units")),
            "l3m_ad_sales":        safe(ads_l3m.get("Ad Sales")),
    ...
    def slim_sku(r):
        row = {
            ...
            "gK": r2(r.get("gross_clicks")),
            "aS": r2(r.get("ad_spend")),
            "aI": r2(r.get("ad_impressions")),
            "aC": r2(r.get("ad_clicks")),
            "aU": r2(r.get("ad_units")),
            "aX": r2(r.get("ad_sales")),
            # LFM ads (for perf panel LFM column)
            "fU": r2(r.get("lfm_gross_units")),
            "fS": r2(r.get("lfm_gross_sales")),
            "fA": r2(r.get("lfm_ad_spend")),
            "fI": r2(r.get("lfm_ad_impressions")),
            "fC": r2(r.get("lfm_ad_clicks")),
            "fN": r2(r.get("lfm_ad_units")),
            "fX": r2(r.get("lfm_ad_sales")),
            "fK": r2(r.get("lfm_gross_clicks")),
            # L3M ads (for perf panel L3M column)
            "tU": r2(r.get("l3m_gross_units")),
            "tS": r2(r.get("l3m_gross_sales")),
            "tA": r2(r.get("l3m_ad_spend")),
            "tI": r2(r.get("l3m_ad_impressions")),
            "tC": r2(r.get("l3m_ad_clicks")),
            "tN": r2(r.get("l3m_ad_units")),
            "tX": r2(r.get("l3m_ad_sales")),
            "tK": r2(r.get("l3m_gross_clicks")),
            "tf": r.get("top_flag"),
            "fs": r.get("flags"),
        }
        # Drop None values to keep file small
